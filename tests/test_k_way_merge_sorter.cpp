...
TEST_F(KWayMergeSorterMemoryTest, SimpleSmallArraySort) {
    ...
    KWayMergeSorter<int> sorter(*factory, input_id, output_id, 
                                sizeof(int) * 3, 
                                2, 
                                10, 
                                true); 
...
TEST_F(KWayMergeSorterMemoryTest, InvalidKValue) {
    ...
    EXPECT_THROW({
        KWayMergeSorter<int> sorter(*factory, input_id, output_id, 
                                    sizeof(int) * 10, 1, 10, true);
    }, std::invalid_argument);
    
    
    EXPECT_THROW({
        KWayMergeSorter<int> sorter(*factory, input_id, output_id, 
                                    sizeof(int) * 10, 0, 10, true);
    }, std::invalid_argument);
}
...
TEST_F(KWayMergeSorterFileTest, FileBasedSort) {
    ...
    KWayMergeSorter<int> sorter(*factory, input_file, output_file,
                                sizeof(int) * 4, 
                                3, 
                                10,
                                true);
...
TEST_F(KWayMergeSorterMemoryTest, StressTestLargeData) {
    ...
    std::vector<int> test_data = GenerateRandomData(data_size, 0, 10000);
    CreateTestData(input_id, test_data);
